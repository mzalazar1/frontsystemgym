...
const BodySection = () => {
  ...
  useEffect(() => {
    if (selectedCuotaRef.current.dni) {
      ...
      const usuarioEncontradoCuota = cuotas.find(
        (cuota) => cuota.dni === selectedCuotaRef.current.dni
      );

      selectedCuotaRef.current = {
        ...selectedCuotaRef.current,
        data: usuarioEncontradoCuota || null,
      };
  ...
  useEffect(() => {
    if (selectedSocioRef.current.dni) {
      const usuarioEncontradoSocio = socios.find(
        (socio) => socio.dni === selectedSocioRef.current.dni
      );

      selectedSocioRef.current = {
        ...selectedSocioRef.current,
        data: usuarioEncontradoSocio || null,
      };
  ...
  return (
    <div className={styles.cuerpo}>
      ...
      <h2 className={styles.h2}>
        Esta aplicación fue desarrollada por Marcos Zalazar para rendir final de
        Trabajo de campo y Trabajo de Diploma
      </h2>
      <p className={styles.texto}>
        Ingrese su DNI para registrar asistencia y verificar el estado de su
        cuota.
      </p>
      <div className={styles.Estadisticas}>
        <Link to="/estadisticas">
          <Boton tipo="estadisticas" texto="VER ESTADISTICAS" />
        </Link>
      </div>
      <div className={styles.Estadisticas}>
        <Link to="/asistencias">
          <Boton tipo="asistencias" texto="REPORTE ASISTENCIAS" />
        </Link>
      </div>
    </div>
  );
