...
const Formulario = () => {
    ...
    const pagoHandler = async (pago) => {
        setModNewPago(true);
        try {
            await dispatch(POST((pago), 'pagos'));
            setModNewPago(false)
            navigate('/pagos');
        } catch (error) {
            setModNewPago(false)
    ...
    return (
        <div className={styles.frmPago}>
            ...
            }
            {
                modFallaPago ?
                    <Modal
                        texto='Falló al cargar el nuevo pago'
                        tipo='nuevoPago' /> : <div></div>
            }
            <h2>Formulario</h2>
            <form onSubmit={handleSubmit(pagoHandler)}>
                ...
                <div>
                    ...
                    />
                    {errors.metodo && <span className={styles.claseError}>{errors.metodo.message}</span>}
                </div>
